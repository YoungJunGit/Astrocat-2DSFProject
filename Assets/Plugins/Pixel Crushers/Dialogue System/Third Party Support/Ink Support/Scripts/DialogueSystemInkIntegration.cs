...
namespace PixelCrushers.DialogueSystem.InkSupport
{
    ...
    public class DialogueSystemInkIntegration : MonoBehaviour
    {
        ...
        /// <summary>
        /// Removes a story at runtime.
        /// </summary>
        /// <param name="storyTitle">The story's title.</param>
        public virtual void RemoveStory(string storyTitle)
        {
            var story = LookupStory(storyTitle);
            if (story != null) {
                stories.Remove(story);
                UnbindExternalFunctions(story);
            }
            var conversation = DialogueManager.masterDatabase.GetConversation(storyTitle);
        ...
        protected virtual void AddStoryToDatabase(string storyTitle, Story story, int conversationID)
        {
            ...
            if (debug) Debug.Log($"Dialogue System: Create conversation [{conversationID}] {storyTitle}");
        ...
        protected virtual void OnConversationStart(Transform actorTransform)
        {
            for (int i = 0; i < inkJSONAssets.Count; i++)
            {
                if (string.Equals(inkJSONAssets[i].name, DialogueManager.lastConversationStarted))
                {
                    ...
                    if (debug) Debug.Log($"Dialogue System: [{Time.frameCount}] Start [{DialogueManager.lastConversationID}] {DialogueManager.lastConversationStarted} with actorID={currentStoryPlayerID}/conversantID={currentStoryActorID}");
        ...
        protected virtual void OnPrepareConversationLine(DialogueEntry entry)
        {
            ...
            if (debug) Debug.Log($"Dialogue System: [{Time.frameCount}] PrepareLine [{entry.conversationID}:{entry.id}]");
            ...
            {
                if (debug) Debug.Log($"Dialogue System: [{Time.frameCount}] Loopback from forced choice [{entry.conversationID}]");
            ...
            else
            {
                ...
                if (debug) Debug.Log($"Dialogue System: [{Time.frameCount}] ChooseChoice [{entry.conversationID}:{entry.id}] {choiceIndex}");
                ...
                {
                    //Debug.Log($"<color=magenta>{activeStory.currentChoices[choiceIndex].text}</color>");
        ...
        protected virtual void BindExternalFunctions(Story story)
        {
            ...
            story.BindExternalFunction("Lua", (string luaCode) => { return Lua.Run(luaCode).asString; });
        }
        ...
        protected virtual void UnbindExternalFunctions(Story story)
        {
            ...
            story.UnbindExternalFunction("Lua");
        }
