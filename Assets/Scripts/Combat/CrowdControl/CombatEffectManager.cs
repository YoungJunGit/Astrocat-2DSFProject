...
{
    public IEffectable AddCombatEffect(EffectInfo info, EffectContext context);
}
...
{
    private readonly Dictionary<string, Func<IEffectable>> _effectFactory = new()
    {
    ...
    public IEffectable AddCombatEffect(EffectInfo info, EffectContext context)
    {
        ...
        if (_effectFactory.TryGetValue(info.ID, out var factory) && factory != null)
        {
            combatEffect = factory.Invoke();
            target.combatEffectUnit.Add(info.Name, combatEffect);
        }
