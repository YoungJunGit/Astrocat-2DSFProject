...
{
    public override async UniTask Execute(IUnitActionContext context, IUnitActionEvent unitAction)
    {
        ...
        await base.Execute(context, unitAction);
    }
}
...
class RangeAttack : BaseAttackAction 
{
    public override async UniTask Execute(IUnitActionContext context, IUnitActionEvent unitAction)
    {
        context.Caster.GetAnimationHandler().attack += () => { ShootBullet(context, unitAction); };
        context.Caster.GetAnimationHandler().ChangeAnimation(AnimCombat.ATTACK);

        await base.Execute(context, unitAction);
    }

    private void ShootBullet(IUnitActionContext context, IUnitActionEvent unitAction)
    {
